...
const pastSessions: PastSession[] = [
  ...
  {
    id: "sess-4809",
    title: "Grand oral — méthode et sujet",
...
const REASONS: { id: string; label: string; hint: string }[] = [
  { id: "no-show", label: "Le prof ne s'est pas connecté", hint: "Absence non prévenue à l'heure prévue." },
  { id: "quality", label: "Qualité de la séance insuffisante", hint: "Contenu hors sujet, pédagogie non adaptée." },
  { id: "billing", label: "Erreur de facturation", hint: "Montant, doublon, remboursement manquant." },
...
export default function StudentHelpDisputePage() {
  ...
  const submit = async () => {
    ...
    console.log("[Dispute] submitted", state);
  };
      ...
      <PageHeader
        eyebrow={
          <>
            ...
            <span className="font-medium text-[#0B0B0F]">Réponse sous 4 h ouvrées</span>
          </>
        }
        title="Ouvrir un litige"
        subline="Décris le problème en 3 étapes. Notre équipe traite ta demande sous 4 h."
      />
      <div>
        <StepIndicator
  ...
  return (
    <div className="mx-auto w-full max-w-[720px]">
      ...
      <p className="mt-3 text-[11px] text-[#8A8D93]">
        ...
        nécessaires avec le prof concerné pour instruire ta demande.
      </p>
    </div>
  );
}
...
function StepSummary({
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-[12px] text-[#6E7178]">
        Un dernier coup d'œil avant l'envoi à notre équipe support.
      </p>
        ...
        <SummaryRow label="Séance">
          {session ? (
            <span className="flex items-center gap-2">
              <Avatar initials={session.teacherInitials} tone="brand" size={28} />
              <span>
                <span className="block text-[13px] font-semibold text-[#0B0B0F]">
                  {session.title}
                </span>
                <span className="block text-[11px] text-[#8A8D93]">
                  {session.teacher} · {session.date}
                </span>
              </span>
            </span>
          ) : (
